...
class TeacherPage extends Component {
  ...
  zipFileSelectorHandler = (event) => {
    const selectedfile = { ...this.state.Form };
    selectedfile.zip.value = event.target.files[0];
    selectedfile.zip.name = URL.createObjectURL(event.target.files[0]);
    this.setState({ Form: selectedfile });
  };
  ...
  render() {
    ...
    if (this.state.Form.zip.value) {
      zipFileName = this.state.Form.zip.value.name;
    }
          ...
          <div className="Teacher-Head-Class">
            <label className="custom-image-upload">
              <input
                ...
                name="zip"
                accept=".zip"
                onChange={this.zipFileSelectorHandler}
              />
              Upload Files
            </label>

            <p className="ImageName">{zipFileName}</p>
